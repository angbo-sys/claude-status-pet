...
function usage() {
  console.log("Usage: pet-switch.js [--dir <install-dir>] [--list] <pet>");
}
...
function main() {
  ...
  if (hasFlag("--list")) {
    for (const pet of pets) {
      console.log(pet.description ? `${pet.name}\t${pet.description}` : pet.name);
    }
    return;
  }
  ...
  writeJsonAtomic(configFile, compactConfig(config));
