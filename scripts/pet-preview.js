...
const STATES = ["idle", "listening", "thinking", "tool", "running", "editing", "searching", "browsing", "delegating", "compacting", "success", "error", "waiting", "sleeping"];
const ACTION_STATES = { blink: "idle", listen: "listening", think: "thinking", work: "tool", run: "running", code: "editing", search: "searching", browse: "browsing", cheer: "success", celebrate: "success", shake: "error", wait: "waiting", sleep: "sleeping", nap: "sleeping" };
...
function positionalArgs() {
  ...
  for (let i = 2; i < process.argv.length; i += 1) {
    ...
    if (arg === "--dir" || arg === "--state" || arg === "--action" || arg === "--cycles") { i += 1; continue; }
...
function previewPet(installDir, name, useColor) {
  if (!isSafePetId(name)) { console.error(`Invalid pet name: ${name}`); process.exitCode = 1; return; }
  const file = path.join(installDir, "pets", `${name}.json`);
  const pet = readJson(file, null);
  if (!pet || !pet.states) { console.error(`Unknown pet: ${name}`); process.exitCode = 1; return; }
...
function animatePet(installDir, name, useColor, state, cycles) {
  if (!isSafePetId(name)) { console.error(`Invalid pet name: ${name}`); process.exit(1); }
  const file = path.join(installDir, "pets", `${name}.json`);
  const pet = readJson(file, null);
  const frames = pet?.states?.[state];
  if (!frames) { console.error(`Unknown pet or state: ${name}/${state}`); process.exit(1); }
...
function main() {
  ...
  if (names.length === 0) { console.error(`No pets found in ${path.join(installDir, "pets")}`); process.exit(1); }
