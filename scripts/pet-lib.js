...
function normalizeConfig(config) {
  ...
  return {
    ...
    displayMode: ["quiet", "compact", "balanced", "full"].includes(merged.displayMode) ? merged.displayMode : base.displayMode,
...
const I18N = {
  en: {
    labels: {
      idle: "idle", waking: "waking", listening: "listening", waiting: "waiting",
      done: "done", "agent done": "agent done", "running shell": "running shell",
      testing: "testing", installing: "installing", reviewing: "reviewing",
      git: "git", editing: "editing", searching: "searching", browsing: "browsing",
      delegating: "delegating", "using tool": "using tool", "tool failed": "tool failed",
      "tool done": "tool done", compacting: "compacting", thinking: "thinking",
      sleeping: "sleeping", napping: "napping"
    },
    moods: {
      tired: "tired", happy: "happy", lonely: "lonely", careful: "careful",
      confident: "confident", busy: "busy", sleepy: "sleepy", focused: "focused",
      fresh: "fresh", curious: "curious", wired: "wired", proud: "proud",
      frazzled: "frazzled", rested: "rested", hungry: "hungry"
    },
    units: {
      moves: "moves", edits: "edits", fails: "fails", streak: "streak",
      tasks: "tasks", ctx: "ctx", energy: "E", bond: "B", clean: "clean"
    },
    care: {
      feed: "Snack accepted.", play: "Play break logged.", nap: "Nap time.",
      pet: "Tiny pat registered.", status: "Status", energy: "Energy",
      bond: "bond", snacks: "snacks", plays: "play", naps: "naps", dataDir: "Data dir"
    }
  },
  zh: {
    labels: {
      idle: "发呆", waking: "醒来", listening: "听着", waiting: "等你",
      done: "完成", "agent done": "分身完成", "running shell": "跑命令",
      testing: "跑测试", installing: "装依赖", reviewing: "看改动",
      git: "整理 Git", editing: "写东西", searching: "找线索",
      browsing: "看网页", delegating: "派分身", "using tool": "用工具",
      "tool failed": "出错了", "tool done": "工具完成", compacting: "收拾记忆",
      thinking: "思考中", sleeping: "睡觉", napping: "小睡"
    },
    moods: {
      tired: "累了", happy: "开心", lonely: "想陪陪", careful: "小心",
      confident: "自信", busy: "忙碌", sleepy: "困了", focused: "专注",
      fresh: "精神", curious: "好奇", wired: "高速", proud: "得意",
      frazzled: "有点乱", rested: "休息好啦", hungry: "想补能量"
    },
    units: {
      moves: "步", edits: "次编辑", fails: "次失败", streak: "连胜",
      tasks: "任务", ctx: "上下文", energy: "能", bond: "亲", clean: "干净"
    },
    care: {
      feed: "投喂成功。", play: "玩耍了一会儿。", nap: "进入小睡。",
      pet: "摸摸完成。", status: "状态", energy: "能量",
      bond: "亲密度", snacks: "零食", plays: "玩耍", naps: "小睡", dataDir: "数据目录"
    }
  }
};
...
function pruneState(state, now = nowMs()) {
  ...
  return { ...state, sessions: Object.fromEntries(sessions), daily };
}

function updateStats(previous, input, status, now = nowMs()) {
  const stats = {
    started_at: previous?.stats?.started_at || now,
    tools: previous?.stats?.tools || 0, edits: previous?.stats?.edits || 0,
    searches: previous?.stats?.searches || 0, shells: previous?.stats?.shells || 0,
    delegates: previous?.stats?.delegates || 0, tests: previous?.stats?.tests || 0,
    installs: previous?.stats?.installs || 0, git: previous?.stats?.git || 0,
    failures: previous?.stats?.failures || 0, success_streak: previous?.stats?.success_streak || 0,
    last_tool_at: previous?.stats?.last_tool_at || 0
  };
  if (input.hook_event_name === "PreToolUse") {
    const kind = toolKind(input.tool_name);
    const bash = kind === "shell" ? bashKind(commandFromInput(input)) : "";
    stats.tools += 1; stats.last_tool_at = now;
  ...
  if (input.hook_event_name === "PostToolUse") {
    if (status.state === "error") { stats.failures += 1; stats.success_streak = 0; }
    else { stats.success_streak += 1; }
  }
  return stats;
}

function accumulateDaily(previousDaily, input, status, now = nowMs()) {
  const key = dayKey(new Date(now));
  const daily = { ...(previousDaily || {}) };
  const item = {
    date: key, tools: daily[key]?.tools || 0, edits: daily[key]?.edits || 0,
    searches: daily[key]?.searches || 0, shells: daily[key]?.shells || 0,
    tests: daily[key]?.tests || 0, installs: daily[key]?.installs || 0,
    git: daily[key]?.git || 0, failures: daily[key]?.failures || 0, successes: daily[key]?.successes || 0
  };
  if (input.hook_event_name === "PreToolUse") {
    const kind = toolKind(input.tool_name);
    const bash = kind === "shell" ? bashKind(commandFromInput(input)) : "";
...
function defaultCare(now = nowMs()) {
  return { energy: 80, affection: 50, snacks: 0, play: 0, naps: 0, last_interaction: now };
}
...
function updateCare(previous, input, status, now = nowMs()) {
  ...
  if (input.hook_event_name === "PreToolUse") care.energy = clamp(care.energy - 1, 0, 100);
  if (status.state === "success") care.affection = clamp(care.affection + 1, 0, 100);
...
function applyCareAction(care, action, now = nowMs()) {
  ...
  if (action === "feed") { next.energy = clamp(next.energy + 18, 0, 100); next.affection = clamp(next.affection + 3, 0, 100); next.snacks = (next.snacks || 0) + 1; }
  else if (action === "play") { next.energy = clamp(next.energy - 8, 0, 100); next.affection = clamp(next.affection + 12, 0, 100); next.play = (next.play || 0) + 1; }
  else if (action === "nap") { next.energy = clamp(next.energy + 28, 0, 100); next.naps = (next.naps || 0) + 1; }
  else if (action === "pet") { next.affection = clamp(next.affection + 8, 0, 100); }
...
function petLevel(status) {
  ...
  const streak = Number(stats.success_streak || 0);
  const score = Math.floor((care.affection + tools + streak * 3) / 50) + 1;
  const level = clamp(score, 1, 4);
  const names = {
    1: { en: "hatchling", zh: "新来的" },
    2: { en: "helper", zh: "小助手" },
    3: { en: "companion", zh: "老搭档" },
    4: { en: "copilot", zh: "王牌搭子" }
  };
  return { level, names: names[level] };
}

const PALETTES = {
  mono: { pet: "0", label: "0", mood: "0", tool: "0", muted: "0", success: "0", error: "0", flair: "0" },
  classic: { pet: "36", label: "33", mood: "35", tool: "90", muted: "90", success: "32", error: "31", flair: "36" },
  soft: { pet: "38;5;110", label: "38;5;180", mood: "38;5;175", tool: "38;5;245", muted: "38;5;244", success: "38;5;114", error: "38;5;203", flair: "38;5;109" },
  terminal: { pet: "32", label: "92", mood: "36", tool: "90", muted: "90", success: "92", error: "91", flair: "32" },
  alert: { pet: "36", label: "37", mood: "33", tool: "90", muted: "90", success: "32", error: "1;31", flair: "35" }
};
...
function moodFor(status, now = nowMs()) {
  ...
  if (["running", "editing", "searching", "browsing", "delegating", "tool"].includes(status.state) && stats.tools >= 5) return "busy";
...
function truncate(input, width) {
  ...
  for (let i = 0; i < text.length; ) {
    ...
    if (ansi) { output += ansi[0]; i += ansi[0].length; continue; }
...
module.exports = {
  basename, bundledPets, color, configPath, dataDir, defaultConfig, displayWidth, ensureDir,
  achievementsFor, accumulateDaily, bar, bashKind, commandFromInput, deterministicPick,
  installedPets, isSafePetId, listPetsInDir, loadConfig, loadPet, mergeConfig, normalizeConfig,
  locale, moodFor, nowMs, palette, petExists, petLevel, pruneState, dayKey, pluginRoot,
  readJson, readStdinJson, sessionKey, shouldShowFlair, shellEscapeSingle, statePath,
  statusFromHook, truncate, translate, applyCareAction, careFor, updateStats, updateCare,
  validatePet, writeJsonAtomic
};
